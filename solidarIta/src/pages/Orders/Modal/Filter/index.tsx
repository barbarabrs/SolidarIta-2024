...
function FilterModal({ onPage }: FilterModalProps) {
  ...
  const [accountList, setAccountList] = useState<dateAccount[]>([
    { value: "", label: "" },
  ]);
  ...
  const handleFilter = (event: FormEvent) => {
    ...
    //Verifica se existe filtro aplicado, para mudança de estado do botão, AO ADICINAR MAIS CAMPOS, COLOCAR NA COMPARAÇÃO!!
    if (filterInputCode) {
      setExistFilter(true);
    } else {
  ...
  return (
    <>
      {!existFilter ? (
        <Button className="color_main" variant="outline" onClick={onOpen}>
          Filtrar
        </Button>
      ) : (
        <Button
        ...
        <ModalContent color={useColorModeValue("gray.800", "white")}>
          <form onSubmit={handleFilter}>
            ...
            <ModalBody>
              <FormControl mt={4} isRequired>
                ...
                <Select
                  ...
                  defaultValue={accountList.filter(
                    (elem: dateAccount) => filterInputCode === elem.value
                  )}
                  onChange={(event) => {
                    {
                      event?.value
                        ? setFilterInputCode(event.value)
                        : setFilterInputCode("");
                    }
                  }}
                  options={accountList}
            ...
            <ModalFooter>
              ...
              <Button onClick={handleLimpar} colorScheme="blue" mr={3}>
                Limpar
              </Button>
